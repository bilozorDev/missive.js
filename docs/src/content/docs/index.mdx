...
hero:
    ...
    tagline: Fully-typed Service Bus for Node.js fueled by fancy Middlewares, Envelopes and Stamps.
    image:
        file: ../../assets/bus.svg
    actions:
        - text: Get Started
...
<CardGrid>
    ...
    <Card title="Webhook Middleware" icon="rocket">
        <p> The Webhook Middleware is going to send the envelope(s) to confirured webhook(s) (post processing). It's another powerful way to decouple your system and to make it more resilient or to simply add Observability!.</p>
    ...
    <Card title="Lock Middleware" icon="seti:lock">
     ...
     <Card title="Mock Middleware" icon="seti:purescript">
        <p>This middleware is pretty useful in development mode, to mock the result of a specific intent to bypass the handler.</p>
        <LinkButton href={'/missive.js/built-in-middlewares/mocker'} variant="secondary" class="float-right" title={"Read the doc about the Mocker Middleware"}>Read the doc</LinkButton>
    </Card>
